...
aps_failure_data_clean$class <-
  as.numeric(aps_failure_data_clean$class)
aps_failure_data_clean$aa_000 <-
  as.numeric(aps_failure_data_clean$aa_000)
aps_failure_data_clean$ac_000 <-
  as.numeric(aps_failure_data_clean$ac_000)
...
for (i in 1:10) {
  ...
  training_errors[i] <-
    sum((net_result_train$net.result[, 1] > 0.5)
        != training_data[, "class"])
  ...
  neural_net4a <- update(neural_net4a, training_data[],
                         training_data[, "class"])
}
...
plot(
  ...
  xlab = "# of Epochs",
  ylab = "# of Errors",
  ylim = c(0, max(training_errors, testing_errors)),
  main = "Panel 2"
)
